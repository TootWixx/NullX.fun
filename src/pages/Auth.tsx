...
export default function Auth() {
  ...
  useEffect(() => {
    const savedEmail = localStorage.getItem('nova_saved_email');
    if (savedEmail) {
      setEmail(savedEmail);
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      localStorage.setItem('nova_saved_email', email);
    }
    ...
    if (error) {
      ...
      toast({
        ...
        description: 'You are on the Starter plan. Check your email to confirm, then open a Discord ticket to upgrade.',
      });
    }
  };
      ...
      <div className="w-full max-w-md space-y-8">
        <div className="flex flex-col items-center gap-3 text-center">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 ring-1 ring-primary/20">
            <img src="/nullx-logo.png" alt="NullX.fun" className="h-8 w-8" />
          </div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ lineHeight: '1.1' }}>NullX.fun</h1>
          <p className="text-sm text-muted-foreground">Roblox script authentication & protection</p>
        </div>

        <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
          <CardHeader className="pb-4">
            <CardTitle className="text-lg">{isLogin ? 'Welcome back' : 'Create account'}</CardTitle>
            <CardDescription>{isLogin ? 'Sign in to manage your scripts' : 'Start protecting your Lua scripts'}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="bg-background/50 pr-10" />
